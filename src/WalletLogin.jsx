...
export default function WalletLogin() {
  ...
  async function connectHandler() {
    ...
    try {
      ...
      // 2) Signieren mit Fehlerbehandlung
      let signature;
      try {
        signature = await signer.signMessage(`Bitte bestätige: ${nonce}`);
      } catch (err) {
        if (err.code === 4001) {
          alert("✋ Signatur abgelehnt.");
          return;
        }
        throw err;
      }
  ...
  return (
    <>
      <div style={{ position: 'relative', maxWidth: '400px', margin: '2rem auto', textAlign: 'center', fontFamily: 'Arial, sans-serif' }}>
        ...
        <button onClick={connectHandler} style={{ width
