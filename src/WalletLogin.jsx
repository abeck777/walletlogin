...
export default function WalletLogin() {
  ...
  const translations = {
    ...
    it: { header: "🔐 Connetti portafoglio", guidePrefix: "Non hai ancora un portafoglio? ", guideLink: "Ecco una guida di 2 minuti.", connect: "Connetti con {name}", back: "Torna alla página di accesso", cookie: "Questo sito utilizza i cookie per migliorare la tua esperienza.", accept: "Accetta" },
    es: { header: "🔐 Conectar billetera", guidePrefix: "¿No tienes cartera? ", guideLink: "Aquí tienes una guía de 2 minutos.", connect: "Conectar con {name}", back: "Volver a la página de inicio de sesión", cookie: "Este sito utilizza cookies per migliorare tu experiencia.", accept: "Aceptar" },
    pt: { header: "🔐 Conectar carteira", guidePrefix: "Ainda não tem uma carteira? ", guideLink: "Aqui está um guia de 2 minutos.", connect: "Conectar com {name}", back: "Voltar à página de login", cookie: "Este sitio usa cookies para mejorar su experiencia.", accept: "Aceitar" },
    ja: { header: "🔐 ウォレットを接続", guidePrefix: "まだウォレットがないですか？ ", guideLink: "2分ガイドはこちら。", connect: "{name}で接続", back: "ログインページに戻る", cookie: "この sito utilizza i cookie per migliorare l’esperienza utente.", accept: "同意する" },
    hi: { header: "🔐 वॉलेट कनेक्ट करें", guidePrefix: "अभी तक वॉलेट नहीं है? ", guideLink: "यहाँ 2-मिनट मार्गदर्शिका है।", connect: "{name} से कनेक्ट करें", back: "ログिन पेज पर वापस जाएँ", cookie: "यहサイト クッキー का उपयोग करता है।", accept: "स्वीकार करें" },
    af: { header: "🔐 Sluit beursie aan", guidePrefix: "Nog geen beursie? ", guideLink: "Hier is ’n 2-minuut gids.", connect: "Verbind met {name}", back: "Terug na aanmeldbladsy", cookie: "Hierdie sito gebruik koekies om jou ervaring te verbeter.", accept: "Aksepteer" }
  };
  ...
  async function connectHandler() {
    ...
    if (connector === "metamask") {
      ...
      const wc = await EthereumProvider.init({ projectId: process.env.REACT_APP_WC_PROJECT_ID, rpcMap: {1: process.env.REACT_APP_INFURA_URL}, chains:[1], showQrModal: true });
      await wc.enable();
      provider = new ethers.BrowserProvider(wc);
    } else {
      const cbWallet = new CoinbaseWalletSDK({ appName:"MeinShop", darkMode:false, jsonRpcUrl: process.env.REACT_APP_INFURA_URL, chainId:1 });
      const cbProv = cbWallet.makeWeb3Provider();
    ...
    if (connector === "metamask" || connector === "coinbase") {
      try {
        ...
        if (err.code === 4001) { alert("✋ Signatur abgelehnt – bitte bestätige in deiner Wallet."); return; }
        throw err;
      }
    }
    ...
    try {
      ...
      const verifyRes = await fetch(`/api/verify`, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({token,address,signature,nonce}) });
      const verifyJson = await verifyRes.json();
      if (!verifyRes.ok || !verifyJson.success) throw new Error("Signatur ungültig – bitte erneut versuchen.");
  ...
  return (
    <>
      <div style={{position:'relative',maxWidth:'400px',margin:'2rem auto',textAlign:'center',fontFamily:'Arial, sans-serif'}}>
        ...
        <button onClick={()=>window.location.href='/'} style={{marginTop:'2rem',padding:'8px 16px',fontSize:'14px',backgroundColor:'#eee',color:'#222',border:'1px solid #ccc',borderRadius:'4px',cursor:'pointer'}}>{t.back}</button>
      </div>
